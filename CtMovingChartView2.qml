...
ChartView{
    ...
    //关于y轴的属性
    property alias y1轴_起始: id接口.y1轴_起始
    property alias y1轴_初始跨度: id接口.y1轴_初始跨度
    property alias y2轴_起始: id接口.y2轴_起始
    property alias y2轴_初始跨度: id接口.y2轴_初始跨度

    //关于线属性
    property alias 线1名: id线1.name
    property alias 线1颜色: id线1.color
    property alias 线2名: id线2.name
    property alias 线2颜色: id线2.color

    //关于点属性
    property alias 点_新y1坐标: id接口.点_新y1坐标  //线1下一个点的y坐标，直接由外部给出
    property alias 点_新y2坐标: id接口.点_新y2坐标  //线2下一个点的y坐标，直接由外部给出
    ...
    Item{
        ...
        //关于y轴的属性
        property double y1轴_起始: 0
        property int y1轴_初始跨度: 100
        property double y2轴_起始: 0
        property int y2轴_初始跨度: 100

        //关于点属性
        property double 点_x坐标: 0  //当前最新的一个点的x坐标，用于与步进计算新的点x坐标
        property double 点_新y1坐标: 25  //线1下一个点的y坐标，直接由外部给出
        property double 点_新y2坐标: 75  //线2下一个点的y坐标，直接由外部给出

        function fun定时器触发(){
            //每触发一次，新加一个点，新点x坐标由步进得出，y坐标由外部给出
            点_x坐标 += x轴_步进
            id线1.append(点_x坐标, 点_新y1坐标)
            id线2.append(点_x坐标, 点_新y2坐标)
