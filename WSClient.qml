...
Item{
    ...
    property alias msgToSend: id接口.msgToSend
    ...
    Item{
        ...
        property string msgToSend: "";
        property int sendCnt: 0
    }
    ...
    Grid{  //可视组件
        ...
        Switch{  //循环发送开关
            ...
            onClicked: {
                ...
                {
                case true:{  //点击后为true时
                    //checked = true  //这个赋值可能只是虚晃一枪
                    if(root.state != "open")
                    {
                        checked = false
                        ToolTip.show("当前未连接服务器", 3000);
                        return;
                    }
                    else if(id接口.msgToSend == "")
                    {
                        checked = false
                        ToolTip.show("要发送的数据为空", 3000);
                        return;
                    }
    ...
    states:[  //状态，用于改变组件是否可视
        ...
        State {  //此为暂时状态，很快会变成open 或 not open
            name: "中断发送"
            PropertyChanges {
                target: idSwch
                checked: false
